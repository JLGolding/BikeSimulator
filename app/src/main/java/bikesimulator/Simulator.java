...
public class Simulator {
    ...
    public void run(){
        ...
        while(running){
            ...
            var commandInput = elements[0];
            if(inputValidator.validate(commandInput)){
                ...
                if(bikePlacedInitially){
                    if(commandInput.equals("GPS_REPORT")){
                    ...
                    if(commandInput.contains("TURN")){
                        var direction = commandInput.split("_")[1];
                        bike.turn(Directions.valueOf(direction));    
                    }
                    if(commandInput.equals("FORWARD")){
                        area.moveBike(bike);
                    }
                }
